...
const Dashboard = () => {
  ...
  const fetchData = async (page, searchQuery = '', selectedOption = '') => {
    try {
      const response = await fetch(`http://localhost:3001/getData?page=${page}&search=${searchQuery}&option=${selectedOption}`);
  ...
  return (
    <div className="App">
      <SearchBar onSearch={(searchQuery, selectedOption) => fetchData(1, searchQuery, selectedOption)} />
